...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const uppered = messages.map((message: string): string =>
        message.endsWith("!") ? message.toUpperCase() : message,
    );
    return uppered.filter((message: string): boolean => !message.endsWith("?"));
};
...
export function injectPositive(values: number[]): number[] {
    const result = [...values];
    const firstNegIndex = result.findIndex((num) => num < 0);
    if (firstNegIndex === -1) {
        const sum = result.reduce((acc, num) => acc + num, 0);
        return [...result, sum];
    } else {
        const sumBeforeNeg = result
